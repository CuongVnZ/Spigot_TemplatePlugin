...
public class ManagerInstances {
    ...
    private static HashMap<Class<? extends AbstractManager>, ExamplePlugin> associatedPlugin = new HashMap<Class<? extends AbstractManager>, ExamplePlugin>();

    protected static final void registerManager(Class<? extends AbstractManager> clazz, AbstractManager instance, ExamplePlugin pl) throws Exception {
    ...
    protected static final void unloadManager(Class<? extends AbstractManager> clazz) {
        if (instances.containsKey(clazz)) {
            ...
            ExamplePlugin pl = associatedPlugin.remove(clazz);
            manager.unload(pl);
    ...
    protected static final void cleanup() {
        for (Entry<Class<? extends AbstractManager>, AbstractManager> e : instances.entrySet()) {
            ExamplePlugin pl = associatedPlugin.get(e.getKey());
