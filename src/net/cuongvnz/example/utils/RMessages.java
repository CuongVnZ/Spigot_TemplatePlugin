...
import net.cuongvnz.core.options.Option;
...
public class RMessages {
    ...
    public static void death(String s) {
        for (Player p : NeoRPGCore.plugin.getServer().getOnlinePlayers()) {
            if (NeoRPGCore.plugin.getPD(p) != null && NeoRPGCore.plugin.getPD(p).getOption(Option.DEATH_ALERTS)) {
                p.sendMessage("");
                p.sendMessage(s);
            }
        }
    ...
    public static Class<?> getNMSClass(String name) {
        String version = NeoRPGCore.plugin.getServer().getClass().getPackage().getName().split("\\.")[3];
        try {
