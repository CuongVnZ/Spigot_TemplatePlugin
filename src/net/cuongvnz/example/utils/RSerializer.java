...
public class RSerializer {
    ...
    public static Location deserializeLocation(String s) {
        Location def = NeoRPGCore.plugin.getServer().getWorlds().get(0).getSpawnLocation();
        if (s.length() == 0)
            return def;
        ...
        try {
            ...
            World w = NeoRPGCore.plugin.getServer().getWorld(data[5]);
            if (w == null) {
                NeoRPGCore.plugin.getServer().getWorlds().get(0).getSpawnLocation();
            }
